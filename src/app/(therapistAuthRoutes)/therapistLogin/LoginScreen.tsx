...
import { setTherapistUserDetails } from '@/reduxStore/reduxExports';
...
//   },[])
  ...
  const handleTherapistLogin = async (req: ITherapistLoginRequest)=>{
    setShowLoginLoader(true);
    const res = await authService.handleTherapistLogin(req);    
    console.log('res: ', res);
    if (res?.status === 200 || res?.status === 202) {
      LocalStorage.save('therapistAccessToken', res?.data?.therapistDetails.expertId);
      await dispatch(setTherapistUserDetails(res?.data?.therapistDetails))
      setShowLoginLoader(false);
      console.log('routing to Dashboard')
      router.replace('/therapistHome')
    } else {
      setShowLoginLoader(false);
      // setErrorMessage('Something went wrong.');
    }
        ...
        <div className="crm__login__card">
          ...
          <div className="crm__login__form__field__wrap">
            ...
            <CustomButton
              ...
              handleClick={async () => { 
                console.log('click');
                handleTherapistLogin({email: emailId, password: password});
